...
export default function PayrollTemplateManager({ userAddress }: Props) {
  ...
  return (
    <div className="border border-[#333] bg-[#111] p-5 space-y-4">
      ...
      {showForm && (
        <div className="border border-[#333] p-4 space-y-4 bg-[#0a0a0a]">
          ...
          <div className="flex gap-3">
            <div className="flex flex-col gap-1">
              <label className="text-[10px] text-[#777] uppercase tracking-widest">Cadence</label>
              <select
                value={form.cadence}
                onChange={(e) =>
                  setForm({
            ...
            <div className="flex flex-col gap-1">
              <label className="text-[10px] text-[#777] uppercase tracking-widest">Currency</label>
              <select
                value={form.currency}
              ...
              <div key={idx} className="border border-[#444] p-3 space-y-2">
                <div className="grid grid-cols-3 gap-2">
                  <input
                    ...
                    onChange={(e) => updateRecipient(idx, 'institution', e.target.value)}
                    placeholder="Institution"
                    className="bg-[#111] border border-[#333] px-2 py-1 text-xs text-white"
                  />
                  <input
                    value={recipient.accountIdentifier}
                    onChange={(e) => updateRecipient(idx, 'accountIdentifier', e.target.value)}
                    placeholder="Account #"
                    className="bg-[#111] border border-[#333] px-2 py-1 text-xs text-white"
                  />
                  <input
                    value={recipient.accountName}
                    onChange={(e) => updateRecipient(idx, 'accountName', e.target.value)}
      ...
      {templates.length === 0 ? (
        <p className="text-xs text-[#555] text-center py-4">No payroll templates yet</p>
      ) : (
        <div className="space-y-2">
          {templates.map((t) => (
            <div
              key={t.id}
              className={cn('border p-3', t.paused ? 'border-[#333] opacity-60' : 'border-[#444]')}
            >
              <div className="flex items-start justify-between">
                <div className="space-y-1">
                  <div className="text-xs text-white font-medium">{t.name}</div>
                  {t.description && <div className="text-[10px] text-[#555]">{t.description}</div>}
                  <div className="text-[10px] text-[#777]">
                    {t.recipients.length} recipients · {t.totalAmount} {t.currency} · {t.cadence}
                  </div>
                </div>
                <div className="flex gap-2">
                  <button
