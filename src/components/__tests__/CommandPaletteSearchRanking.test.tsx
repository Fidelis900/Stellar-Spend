...
describe('CommandPalette Search Ranking', () => {
  ...
  it('ranks starts-with label match second', () => {
    ...
    const startsWith = labels.findIndex((el) => el.textContent?.includes('homepage'));
  ...
  it('ranks keyword exact match high', () => {
    ...
    const labels = options.map((opt) => within(opt).getByText(/./));
    const keywordExact = labels.findIndex((el) => el.textContent?.includes('Dashboard'));
  ...
  it('ranks contains match lower than starts-with', () => {
    ...
    const startsWithIndex = labels.findIndex((text) => text.includes('homepage'));
    const containsIndex = labels.findIndex((text) => text.includes('go home page'));
