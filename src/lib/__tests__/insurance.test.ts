  ...
  describe("Integration Tests: Claim Eligibility & Workflow", () => {
    ...
    it("should file claim successfully on existing insurance policy fixture", async () => {
      ...
      expect(db.query).toHaveBeenCalledWith(
        ...
        expect.arrayContaining([expect.stringMatching(/^CLAIM-/), "Transaction lost", "tx_hash_proof", mockInsuranceId])
      );
    ...
    it("should reject claim with specific rejection reason", async () => {
      (db.query as any).mockResolvedValueOnce({
        rows: [{ ...mockClaimedPolicyRow, status: "claim_rejected", rejection_reason: "Insufficient evidence" }],
      });
    ...
    it("should process insurance payout for approved claims", async () => {
      ...
      (db.query as any)
        ...
        .mockResolvedValueOnce({ rows: [{ ...approvedRow, status: "paid", payout_reference: "PAY-123" }] }); // payout UPDATE
    ...
    it("should throw error if attempting payout on unapproved claim", async () => {
      ...
      await expect(processInsurancePayout(mockInsuranceId)).rejects.toThrow("Claim must be approved before payout");
    });
